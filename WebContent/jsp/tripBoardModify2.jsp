...
<body id="myPage" data-spy="scroll" data-target=".navbar" data-offset="60">	
		...
		<form action="../common/tripBoardModify" method="post" id="modifyboard">
...
<!-- 			</div> -->
		        ...
		        <!-- ================ -->
		            ...
		            <!-- filters start -->
								...
								<div class="form-inline">
									<div class="form-group">
										...
										<div class="input-group date">
											<input type="text" class="form-control" id="trip_board_startdate" name="trip_board_startdate" value="${trip_board.trip_board_startdate}">
										</div>
		              				</div>
									<div class="form-group ml-1">
										<label>Return On</label>
					              		<div class="input-group date">
											<input type="text" class="form-control" id="trip_board_enddate" name="trip_board_enddate" value="${trip_board.trip_board_enddate}">
					              		</div>		              
		              				</div>
		                <div class="form-group ml-1">
		                  <label>Mens</label>
		                  <select name="trip_board_mancount" class="form-control">
		                    <option value="0" <c:if test="${trip_board.trip_board_mancount == 0}">selected</c:if>>남자없음</option>
		                    <option value="1" <c:if test="${trip_board.trip_board_mancount == 1}">selected</c:if>>1 Adult</option>
		                    <option value="2" <c:if test="${trip_board.trip_board_mancount == 2}">selected</c:if>>2 Adults</option>
		                    <option value="3" <c:if test="${trip_board.trip_board_mancount == 3}">selected</c:if>>3 Adults</option>
		                    <option value="4" <c:if test="${trip_board.trip_board_mancount == 4}">selected</c:if>>4 Adults</option>
		                    <option value="5" <c:if test="${trip_board.trip_board_mancount == 5}">selected</c:if>>5 Adults</option>
		                    <option value="6" <c:if test="${trip_board.trip_board_mancount == 6}">selected</c:if>>6 Adults</option>
		                    <option value="7" <c:if test="${trip_board.trip_board_mancount == 7}">selected</c:if>>7 Adults</option>
		                    <option value="8" <c:if test="${trip_board.trip_board_mancount == 8}">selected</c:if>>8 Adults</option>
		                    <option value="9" <c:if test="${trip_board.trip_board_mancount == 9}">selected</c:if>>9 Adults</option>
		                    <option value="10" <c:if test="${trip_board.trip_board_mancount == 10}">selected</c:if>>10 Adults</option>
		                  </select>
		                </div>
		                <div class="form-group ml-1">
		                  <label>Womans</label>
		                  <select name="trip_board_womancount" class="form-control">
		                    <option value="0" <c:if test="${trip_board.trip_board_womancount == 0}">selected</c:if>>여자없음</option>
		                    <option value="1" <c:if test="${trip_board.trip_board_womancount == 1}">selected</c:if>>1 Adult</option>
		                    <option value="2" <c:if test="${trip_board.trip_board_womancount == 2}">selected</c:if>>2 Adults</option>
		                    <option value="3" <c:if test="${trip_board.trip_board_womancount == 3}">selected</c:if>>3 Adults</option>
		                    <option value="4" <c:if test="${trip_board.trip_board_womancount == 4}">selected</c:if>>4 Adults</option>
		                    <option value="5" <c:if test="${trip_board.trip_board_womancount == 5}">selected</c:if>>5 Adults</option>
		                    <option value="6" <c:if test="${trip_board.trip_board_womancount == 6}">selected</c:if>>6 Adults</option>
		                    <option value="7" <c:if test="${trip_board.trip_board_womancount == 7}">selected</c:if>>7 Adults</option>
		                    <option value="8" <c:if test="${trip_board.trip_board_womancount == 8}">selected</c:if>>8 Adults</option>
		                    <option value="9" <c:if test="${trip_board.trip_board_womancount == 9}">selected</c:if>>9 Adults</option>
		                    <option value="10" <c:if test="${trip_board.trip_board_womancount == 10}">selected</c:if>>10 Adults</option>
		                  </select>
		                </div>
		                <div class="form-group ml-1">
		                  <label>Childrens</label>
		                  <select name="trip_board_childcount" class="form-control">
		                    <option value="0" <c:if test="${trip_board.trip_board_childcount == 0}">selected</c:if>>아이없음</option>
		                    <option value="1" <c:if test="${trip_board.trip_board_childcount == 1}">selected</c:if>>1 child</option>
		                    <option value="2" <c:if test="${trip_board.trip_board_childcount == 2}">selected</c:if>>2 children</option>
		                    <option value="3" <c:if test="${trip_board.trip_board_childcount == 3}">selected</c:if>>3 children</option>
		                    <option value="4" <c:if test="${trip_board.trip_board_childcount == 4}">selected</c:if>>4 children</option>
		                    <option value="5" <c:if test="${trip_board.trip_board_childcount == 5}">selected</c:if>>5 children</option>
		                    <option value="6" <c:if test="${trip_board.trip_board_childcount == 6}">selected</c:if>>6 children</option>
		                    <option value="7" <c:if test="${trip_board.trip_board_childcount == 7}">selected</c:if>>7 children</option>
		                    <option value="8" <c:if test="${trip_board.trip_board_childcount == 8}">selected</c:if>>8 children</option>
		                    <option value="9" <c:if test="${trip_board.trip_board_childcount == 9}">selected</c:if>>9 children</option>
		                    <option value="10" <c:if test="${trip_board.trip_board_childcount == 10}">selected</c:if>>10 children</option>
		                  </select>
		                </div>
		                <div class="form-group ml-1 pull-right">
		                	<label>Make a board public?</label>
		                	<div class="material-switch input-group pull-right">
		                		<input id="someSwitchOptionSuccess" name="trip_board_bool" type="checkbox" <c:if test="${trip_board.trip_board_bool == 'on'}">checked</c:if> class="form-control"/>
		                ...
		                <div class="form-group ml-1 pull-right">
		                	...
		                	<div class="material-switch input-group pull-right">
		                		<input id="someSwitchOptionSuccess2" name="trip_board_recruit" type="checkbox" <c:if test="${trip_board.trip_board_recruit == 'on'}">checked</c:if> class="form-control"/>
		...
		<input type="hidden" name="trip_board_num" value="${trip_board.trip_board_num}">
		<input type="hidden" name="member_id" value="tester">
		<input type="hidden" name="member_nick" value="testman">
      		...
      		<table class="table table-hover">
      			...
      			<tbody name="trStaff">
      				<c:forEach items="${daily_tripList}" var="daily" varStatus="count">
	      				<tr>
	      					<td>
	      						<input type="button" onclick="lookingRoute(${daily.daily_trip_index})" value="경로보기">
	      						<input type="button" id="saveDailydata${daily.daily_trip_index}" onclick="saveDailydata(${daily.daily_trip_index})" value="일정 저장">
	      					</td>
      ...
      <div class="wrap">	
      	<div class="row text-center">
      	  <div class="col-sm-12">
		      ...
		      <a href="tripBoardList" class="btn btn-primary primary">목록보기</a>
	      </div>
	   </div>
	  </div>
